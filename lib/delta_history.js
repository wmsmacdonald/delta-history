...
DeltaHistory.prototype.addVersion = function(fileId, fileBuffer) {
  ...
  if (this.files[fileId] === undefined) {
    ...
    let fileFolder = path.join(this.rootFolder, 'data/', folder);
  ...
  if (this.files[fileId].versions[versionId] === undefined) {
    fs.writeFileSync(path.join(this.rootFolder, 'data', this.files[fileId].folder, versionId), fileBuffer);
...
DeltaHistory.prototype.getLatestVersion = function(fileId) {
  ...
  let latestBuffer = fs.readFileSync(path.join(this.rootFolder, 'data',  this.files[fileId].folder, this.files[fileId].latest));
  return latestBuffer;
};

DeltaHistory.prototype.getDelta = function(fileId, versionId) {
  if (this.files[fileId] === undefined || this.files[fileId].versions[versionId] === undefined) {
    return null;
  }
